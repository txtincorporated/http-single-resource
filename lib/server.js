...
function start(request, method) {
  ...
  function onRequest(request, response, data) {
    ...
    var book = request.on('data', (chunk) => {
      var bookData = [];
      bookData.push(chunk);
      bookData = Buffer.concat(bookData).toString();
      console.log('ln19 body: ', bookData);
      return bookData;
    });
